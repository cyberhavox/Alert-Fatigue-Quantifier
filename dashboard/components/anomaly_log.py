...
# Severity color tokens (matches theme.css state variables)
...
def render_anomaly_log(anomalies_df: pd.DataFrame) -> None:
    ...
    Args:
        anomalies_df: Flagged anomalies with columns
    ...
    for _, row in display_df.iterrows():
        ...
        rows_html += f"""
        <div style="display:grid; grid-template-columns:170px 110px 1fr 120px 100px 110px;
                    padding:10px 16px; gap:8px; border-bottom:1px solid rgba(255,255,255,0.04);
                    align-items:center; background:{bg}; border-left:3px solid {color};">
          <span style="font-family:'JetBrains Mono',monospace; font-size:11px;
                       color:var(--text-muted); font-feature-settings:'tnum' 1;">
            {row.get("Timestamp", "—")}
          </span>
          <span style="font-family:'JetBrains Mono',monospace; font-size:12px;
                       color:var(--text-primary); font-feature-settings:'tnum' 1; font-weight:500;">
            {row.get("Analyst", "—")}
          </span>
          ...
          <span style="font-family:'JetBrains Mono',monospace; font-size:12px;
                       color:{color}; font-feature-settings:'tnum' 1;">
            {deviation_str}
          </span>
          <span style="font-family:'JetBrains Mono',monospace; font-size:12px;
                       color:var(--text-secondary); font-feature-settings:'tnum' 1;">
            {pvalue_str}
          </span>
    ...
    col_header = """
    <div style="display:grid; grid-template-columns:170px 110px 1fr 120px 100px 110px;
                padding:10px 16px; gap:8px; background:rgba(255,255,255,0.04);
                border-bottom:1px solid var(--border-subtle);">
      <span style="font-size:10px; font-weight:500; text-transform:uppercase;
                   letter-spacing:0.8px; color:var(--text-muted);">Timestamp</span>
      <span style="font-size:10px; font-weight:500; text-transform:uppercase;
                   letter-spacing:0.8px; color:var(--text-muted);">Analyst</span>
      <span style="font-size:10px; font-weight:500; text-transform:uppercase;
                   letter-spacing:0.8px; color:var(--text-muted);">Signal</span>
      <span style="font-size:10px; font-weight:500; text-transform:uppercase;
                   letter-spacing:0.8px; color:var(--text-muted);">Deviation (z)</span>
      <span style="font-size:10px; font-weight:500; text-transform:uppercase;
                   letter-spacing:0.8px; color:var(--text-muted);">p-value</span>
      <span style="font-size:10px; font-weight:500; text-transform:uppercase;
                   letter-spacing:0.8px; color:var(--text-muted);">State</span>
    </div>
    """
    ...
    st.markdown(
        f"""
        <div class="anomaly-table-wrap">
          <div style="padding:12px 16px 10px; border-bottom:1px solid var(--border-subtle);
                      display:flex; justify-content:space-between; align-items:center;">
            <span style="font-size:13px; font-weight:400; color:var(--text-primary);">
              Degradation Events
            </span>
            <span style="font-size:11px; color:var(--text-muted);
                         font-family:'JetBrains Mono',monospace;">{count_label}</span>
          </div>
          {col_header}
          {rows_html}
        </div>
